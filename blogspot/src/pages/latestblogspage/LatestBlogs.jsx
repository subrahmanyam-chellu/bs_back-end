    ...
    const getLatestBlogs = async () => {
        try {
            ...
            const result = await axios.get(
                `${import.meta.env.VITE_BACKEND_URL}/getallposts?search=${content}&pageNo=${pageNo}&sortOrder=${sort}&status=posted`,
                    ...
                    <Grid container spacing={2} justifyContent="center">
                        {latestBlogs.map((item) => (
                            <Grid item xs={12} sm={6} md={4} key={item._id}>
                                <MediaCard key={item._id} data={item} edit={false} deletes={false} />
                            </Grid>
                        ))}
                    </Grid>
                </Box>
